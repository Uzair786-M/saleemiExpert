...
export const TestimonialsAdminPage = () => {
  ...
  const handleSave = () => {
    ...
    if (editing !== null) {
      ...
      setItems(prev => prev.map(t => t.id === editing ? { ...t, ...form, rating: Number(form.rating) } : t));
    } else {
      // TODO: POST /api/testimonials
      const newItem = { ...form, id: Date.now(), rating: Number(form.rating) };
      setItems(prev => [...prev, newItem]);
    }
    setForm(empty); setEditing(null); setShowForm(false);
  };

  const handleEdit = (item) => { setForm({ name: item.name, title: item.title || "", review: item.review, rating: item.rating }); setEditing(item.id); setShowForm(true); setError(""); };
    ...
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "2rem", flexWrap: "wrap", gap: "1rem" }}>
        <div>
          ...
          <p style={{ color: "#6b7280", fontSize: "0.875rem" }}>{items.length} total reviews</p>
        </div>
      ...
      {showForm && (
        <div style={{ padding: "1.75rem", borderRadius: "16px", backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(34,211,238,0.3)", marginBottom: "2rem" }}>
          <h3 style={{ color: "#ffffff", fontWeight: 700, marginBottom: "1.25rem" }}>{editing ? "Edit" : "Add"} Testimonial</h3>
          <div style={{ display: "grid", gap: "1rem", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", marginBottom: "1rem" }}>
            <div>
              <label style={{ color: "#9ca3af", fontSize: "0.8rem", display: "block", marginBottom: "6px" }}>Client Name *</label>
              <input name="name" value={form.name} onChange={handleChange} placeholder="John Smith" style={inputStyle} onFocus={e => e.currentTarget.style.borderColor = "#22d3ee"} onBlur={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"} />
            </div>
            <div>
              <label style={{ color: "#9ca3af", fontSize: "0.8rem", display: "block", marginBottom: "6px" }}>Title / Role</label>
              <input name="title" value={form.title} onChange={handleChange} placeholder="Store Owner, USA" style={inputStyle} onFocus={e => e.currentTarget.style.borderColor = "#22d3ee"} onBlur={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"} />
            </div>
            <div>
              <label style={{ color: "#9ca3af", fontSize: "0.8rem", display: "block", marginBottom: "6px" }}>Rating</label>
              <select name="rating" value={form.rating} onChange={handleChange} style={{ ...inputStyle, cursor: "pointer" }}>
                {[5,4,3,2,1].map(r => <option key={r} value={r} style={{ backgroundColor: "#0d1224" }}>{"★".repeat(r)} ({r}/5)</option>)}
              </select>
            </div>
          </div>
          <div style={{ marginBottom: "1rem" }}>
            <label style={{ color: "#9ca3af", fontSize: "0.8rem", display: "block", marginBottom: "6px" }}>Review *</label>
            <textarea name="review" rows="4" value={form.review} onChange={handleChange} placeholder="Client review text..." style={{ ...inputStyle, resize: "none" }} onFocus={e => e.currentTarget.style.borderColor = "#22d3ee"} onBlur={e => e.currentTarget.style.borderColor = "rgba(255,255,255,0.1)"} />
          </div>
        ...
        {items.map(item => (
          <div key={item.id} style={{ padding: "1.5rem", borderRadius: "14px", backgroundColor: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "0.75rem" }}>
              <div>
                <p style={{ color: "#ffffff", fontWeight: 700 }}>{item.name}</p>
                <p style={{ color: "#6b7280", fontSize: "0.8rem" }}>{item.title || "Verified Client"}</p>
              </div>
              <div style={{ display: "flex", gap: "2px" }}>
                {[1,2,3,4,5].map(s => <span key={s} style={{ color: s <= item.rating ? "#f59e0b" : "#374151" }}>★</span>)}
              </div>
            </div>
